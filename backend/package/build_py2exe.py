...
# -*- coding: utf-8 -*- #
#
# Used to buildd the Ardublockly Python server into a Windows executable,
# now it builds the OTOne Python backend into a Windows executable
#
...
def get_python_files_to_compile():
    """
    Returns the scripts to package, ??? only needs the entry point to
    work, everything else is imported.
    """
    start_file = os.path.join(project_root_dir, "otone_client.py")
    return [start_file]
...
def main():
    print(script_tag + "Building OTOne App for Windows.")
